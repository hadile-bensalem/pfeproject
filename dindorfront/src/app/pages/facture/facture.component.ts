...
export class FactureComponent implements OnInit, OnDestroy {
  ...
  bons: FactureClient[]         = [];
  filteredBons: FactureClient[] = [];
  articles: Article[]           = [];
  clients: Client[]             = [];
  stats: FactureClientStats | null = null;
  selectedBon: FactureClient | null = null;
  ...
  printBon: FactureClient | null = null;

  beneficeDateDebut = '';
  beneficeDateFin   = '';
  beneficeBons: FactureClient[] = [];
  ...
  loadAll(): void {
    this.isLoading = true;
    this.errorMessage = '';
    this.svc.getFactures(this.dateDebut, this.dateFin, this.filterMode || undefined, 'BON_LIVRAISON')
    ...
    this.svc.getStats(this.dateDebut, this.dateFin).pipe(takeUntil(this.destroy$)).subscribe({ next: s => this.stats = s });
  }
  ...
  applySearch(): void {
    ...
    this.filteredBons = term
      ? this.bons.filter(b =>
          b.numeroFacture?.toLowerCase().includes(term) ||
  ...
  selectBon(bon: FactureClient): void {
    if (this.selectedBon?.id === bon.id) { this.selectedBon = null; return; }
    this.svc.getById(bon.id).subscribe({ next: full => this.selectedBon = full });
  }
  ...
  openSaisie(): void {
    ...
    this.buildSaisiePaiementForm();
    this.svc.getNextNumeroBL().subscribe({
      next: n => this.nextNumeroBon = n, error: () => this.nextNumeroBon = '—'
    });
    setTimeout(() => document.getElementById('saisie-bl-panel')
  ...
  onClientSelectChange(): void {
    ...
    if (this.selectedClient) {
      this.svc.getTopArticlesClient(this.selectedClient.id, 10).pipe(takeUntil(this.destroy$))
  ...
  private emptyLigneInfo() { return { stockDispo: 0, prixAchat: 0, dernierPrix: 0, found: false }; }
  ...
  openArticlePicker(rowIdx: number): void {
    ...
    setTimeout(() => (document.getElementById('bl-picker-search') as HTMLInputElement)?.focus(), 80);
  }
  ...
  selectArticleFromPicker(a: Article): void {
    ...
    this.svc.getLastPrice(a.codeArticle).subscribe({
      next: res => {
  ...
  focusCell(rowIdx: number, fieldId: string): void {
    setTimeout(() => {
      const el = document.getElementById(`bl-${rowIdx}-${fieldId}`) as HTMLInputElement | null;
    ...
    if (code) {
      ...
      if (a) {
        this.saisieLignes.at(rowIdx).patchValue({
          designation: a.designation, unite: a.unite ?? '', tva: a.tva ?? 0
        });
        const stockDispo = (a.stock1 ?? 0) + (a.stock2 ?? 0);
        const prixAchat  = (a.pump && a.pump > 0) ? a.pump : (a.prixAchatHT ?? 0);
        this.svc.getLastPrice(a.codeArticle).subscribe({
  ...
  onDateEnter(): void { (document.getElementById('bl-client') as HTMLElement)?.focus(); }
  onClientEnter(): void { this.focusCell(0, 'code'); }
  ...
  onSaisieSubmit(): void {
    ...
    this.svc.create(request).subscribe({
      next: bon => {
        ...
        this.saisieSuccess  = `Bon ${bon.numeroFacture} enregistré — ${this.formatNum(bon.netAPayer)} DT`;
  ...
  deleteBon(bon: FactureClient): void {
    if (!confirm(`Supprimer le bon ${bon.numeroFacture} ?`)) return;
    this.svc.delete(bon.id).subscribe({
      next: () => {
        this.bons = this.bons.filter(b => b.id !== bon.id);
        this.applySearch();
        if (this.selectedBon?.id === bon.id) this.selectedBon = null;
        this.svc.getStats(this.dateDebut, this.dateFin).subscribe({ next: s => this.stats = s });
        this.clientService.getAll().subscribe({ next: list => this.clients = list ?? [] });
      }
    });
  }

  setEtatPaiement(bon: FactureClient, etat: string): void {
    this.svc.updateEtatPaiement(bon.id, etat).subscribe({
      next: updated => {
        const idx = this.bons.findIndex(b => b.id === bon.id);
  ...
  loadBenefice(): void {
    ...
    this.svc.getFactures(this.beneficeDateDebut, this.beneficeDateFin, undefined, 'BON_LIVRAISON')
      .pipe(takeUntil(this.destroy$)).subscribe({
  ...
  generateRapport(): void {
    ...
    this.svc.getRapportPeriode(this.rapportClientId, this.rapportDateDebut, this.rapportDateFin)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: rows  => { this.buildPivot(rows); this.rapportLoading = false; this.rapportGenerated = true; },
        error: ()   => { this.rapportLoading = false; this.rapportError = 'Erreur lors du chargement du rapport.'; }
      });
  }
  ...
  private buildPivot(rows: RapportPeriodeRow[]): void {
    ...
    const pivot: { [d: string]: { [desig: string]: { qte: number; total: number } } } = {};
    for (const row of rows) {
      desigSet.add(row.designation);
      dateSet.add(row.date);
      if (!pivot[row.date]) pivot[row.date] = {};
      const cell = pivot[row.date][row.designation];
      if (cell) { cell.qte += +row.quantite; cell.total += +row.totalHT; }
      else pivot[row.date][row.designation] = { qte: +row.quantite, total: +row.totalHT };
    }
  ...
  rapportRowTotal(date: string): number {
    ...
    return row ? Object.values(row).reduce((s, c) => s + c.total, 0) : 0;
  }
  ...
  printRapport(): void {
    ...
    const thDesig = this.rapportDesignations.map(d => `<th colspan="2" class="th-desig">${d}</th>`).join('');
    const thSub   = this.rapportDesignations.map(() => `<th class="th-sub">Qté × PU</th><th class="th-sub">Montant</th>`).join('');
...
<title>Rapport BL — ${this.rapportClientNom}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Arial Narrow',Arial,sans-serif;font-size:9pt;color:#1a1a1a;padding:10mm}
  h2{font-size:13pt;color:#51000d;margin-bottom:3pt}.sub{font-size:9pt;color:#555;margin-bottom:8pt}
  table{width:100%;border-collapse:collapse;margin-bottom:8pt}th,td{border:1pt solid #ccc;padding:3pt 5pt}
  ...
  .th-desig{background:#745b00;color:#fff;text-align:center;font-size:7.5pt}.th-sub{background:#5a4500;color:#fff;text-align:center;font-size:7pt;font-weight:400}
  th[rowspan]{background:#51000d;color:#fff;vertical-align:middle;text-align:center}
  .td-date{font-weight:700;color:#51000d;white-space:nowrap}.td-qtepu{text-align:center;font-size:7.5pt;color:#444;white-space:nowrap}
  .td-mt{text-align:right;font-weight:600}.td-rowtotal{text-align:right;font-weight:700;background:#f5eded}.td-empty{background:#f8f8f8}
  ...
  tfoot .tf-qte{text-align:center;font-size:7.5pt}tfoot .td-label{text-align:center}tfoot .tf-grand{background:#300008;font-size:10pt}
  .total-box{text-align:right;font-size:12pt;font-weight:700;color:#51000d;padding:4pt 0}
</style></head><body>
<h2>Rapport Bons de Livraison par Période</h2>
...
<table><thead><tr><th rowspan="2" style="min-width:75pt">Date</th>${thDesig}<th rowspan="2" style="min-width:70pt">TOTAL</th></tr><tr>${thSub}</tr></thead>
<tbody>${dataRows}</tbody>
<tfoot><tr><td class="td-label">TOTAL</td>${footerCells}<td class="tf-grand">${fmtN(this.rapportGrandTotal)}</td></tr></tfoot></table>
