...
import { TransactionFournisseur } from '../../../models/fournisseur-etat.model';
import { Fournisseur } from '../../../models/fournisseur.model';
import { FournisseurService } from '../../../services/fournisseur.service';

@Component({
  selector: 'app-fournisseur-detail-etat',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fournisseur-detail-etat.component.html',
  styleUrl: './fournisseur-detail-etat.component.css'
})
export class FournisseurDetailEtatComponent implements OnInit, OnDestroy {
  ...
  errorMessage = '';
  searchTerm = '';
  sortColumn: keyof TransactionFournisseur | '' = '';
  ...
  transactionToDelete: TransactionFournisseur | null = null;
  ...
  private loadFournisseur(): void {
    ...
    this.fournisseurService.getById(this.fournisseurId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (f) => this.fournisseur = f,
      error: () => this.fournisseur = null
    });
  }
  ...
  private loadTransactions(): void {
    ...
    this.isLoading = true;
    this.errorMessage = '';
    this.fournisseurService.getTransactionsByFournisseur(this.fournisseurId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
  ...
  private applyFilterAndSort(): void {
    ...
    if (this.sortColumn) {
      list.sort((a, b) => {
        const va = a[this.sortColumn as keyof TransactionFournisseur];
        const vb = b[this.sortColumn as keyof TransactionFournisseur];
        let cmp = 0;
  ...
  sortBy(column: keyof TransactionFournisseur): void {
  ...
  backToEtat(): void {
    this.router.navigate(['/fournisseurs/etat']);
  }

  soldeClass(): string {
    if (this.solde <= 0) return 'solde-ok';
    if (this.solde > 0 && this.solde <= 1000) return 'solde-warning';
    return 'solde-alert';
  }
}
